...
CREATE OR REPLACE PROCEDURE COPY_AUTOMATE (project varchar (100))
    ...
    var return_value = "";
 try {
sql_cmd = `select tbl_name,stage_name,filename,file_fmt from config_tabte
where project = `+`'${PROJECT}'`;
var sql_stmt = snowtlake.createStatement({sqlText:sql_cmd});
var rs=sql_stmt.execute():
while (rs.next())
    {
        tname = rs.getColumnValue(l);
        sname = rs.getColumnValue(2);
        fname = rs.getColumnValue(3);
        fmtname = rs.getColumnValue(4);
        var copy_data =	`COPY INTO `+ tname +` FROM @` + sname + '/ ' + fname + `file_format=`+ fmtname+`;`;
...
-- Working for the exact fields in config_table, for similar files refer below:
...
CREATE OR REPLACE PROCEDURE COPY_AUTOMATE (PROJECT VARCHAR(100))
    ...
    var return_value = "";
    try {
                var sql_cmd = `SELECT TRG_TBL_NAME, stage_name, file_name, file_fmt
                       FROM config_table
                       WHERE PROJECT = '${PROJECT}'`;
        ...
        // Loop through the result set and execute COPY commands
        while (rs.next()) {
            var tname = rs.getColumnValue(1);  // First column: TRG_TBL_NAME
            var sname = rs.getColumnValue(2);  // Second column: stage_name
            var fname = rs.getColumnValue(3);  // Third column: file_name
            var fmtname = rs.getColumnValue(4);  // Fourth column: file_fmt

            // Create the COPY INTO statement dynamically
            var copy_data = `COPY INTO ${tname} FROM @${sname}/${fname} FILE_FORMAT = (FORMAT_NAME = ${fmtname});`;
